...
b = zeros(L, 1);
for i = range
    A(i-28, :) = [g1(thetadd(i)) g3(thetad(i)) g5(5)];
    b(i-28)    = psidd(i) - ro(2)*g2(psi(i), psid(i), psidd(i)) - ro(4)*g4(psid(i));
end
x = (A'*A) \ ((A')*b);
ro(1) = x(1);
ro(3) = x(2);
...
% ro6 ro7 ro8
for i = range
    A(i-28, :) = [g6(thetadd(i)) g7(thetad(i)) g10(5)];
    b(i-28)    = psidd(i) - ro(8)*g8(psid(i), thetad(i)) - ro(9)*g9(psi(i));
end
x = (A'*A) \ ((A')*b);
ro(6) = x(1);
ro(7) = x(2);
